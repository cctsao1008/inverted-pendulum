...
#define SSD1306_CMD_SET_CLOCK            0xD5U
    ...
    uint8_t contrast)
{
    ...
    display->write_command(SSD1306_CMD_DISPLAY_ON, display->context);
...
void ssd1306_write_line(
    ...
    const char *text)
{
    uint8_t row_buffer[SSD1306_WIDTH] = {0U};
    ...
    if ((display == NULL) ||
        (text == NULL) ||
        (row >= SSD1306_PAGE_COUNT)) {
        return;
    }

    text_length = strlen(text);
    if (text_length > SSD1306_LINE_COLUMNS) {
        text_length = SSD1306_LINE_COLUMNS;
    }
    ...
    base = (uint32_t)row * SSD1306_WIDTH;
    if (memcmp(&display->buffer[base], row_buffer, SSD1306_WIDTH) != 0) {
        memcpy(&display->buffer[base], row_buffer, SSD1306_WIDTH);
    ...
    size_t max_data_bytes)
{
        ...
        for (page = 0U; page < SSD1306_PAGE_COUNT; page++) {
            if ((display->dirty_pages & (uint8_t)(1U << page)) != 0U) {
                ...
                display->write_command(
                    (uint8_t)(SSD1306_CMD_PAGE_BASE | page),
                    display->context);
                display->write_command(0x00U, display->context);
                display->write_command(0x10U, display->context);
                break;
            }
        }
    }
    ...
    remaining = SSD1306_WIDTH - display->flush_column;
    transfer = (remaining < max_data_bytes)
        ? remaining
        : max_data_bytes;
    base = ((uint32_t)display->flush_page * SSD1306_WIDTH) +
    ...
    if (display->flush_column >= SSD1306_WIDTH) {
        display->dirty_pages &=
